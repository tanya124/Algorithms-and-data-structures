/*ƒано число N < 106 и последовательность пар целых чисел из [-231, 231] длиной N.
 * ѕостроить декартово дерево из N узлов, характеризующихс€ парами чисел (Xi, Yi).
 *  ажда€ пара чисел (Xi, Yi) определ€ет ключ Xi и приоритет Yi в декартовом дереве.
 * ¬ычислить количество узлов в самом широком слое декартового дерева и количество узлов в самом широком слое наивного дерева поиска.
 * ¬ывести их разницу. –азница может быть отрицательна.*/

/*јлгоритм решени€:
1)Cтроим декртово и наивное дерево поиска
2)Ќаходим рекурсивно их высоту
3)Ќаходим их максимальную ширину следующим образом:
    - создаем массив, который будет хранить количество элементов на определенном уровне дерева (длина массива = высота дерева)
    - делаем обход дерева, заполн€€ массив
    - возвращаем наибольший из элементов массива
*/
